...
def test_pybird():
    ...
    primordial = mimport('cosmopipe.theory.primordial',options={'engine':'eisenstein_hu','compute':'pk_m'})
    primordial.setup()
    primordial.execute()
    data_block = primordial.data_block
    pybird = mimport('cosmopipe.theory.pybird',data_block=data_block)
    ...
    mu = 0.2
    pk1 = data_block[section_names.model,'y_callable'](k,mu)
    #data_block[section_names.galaxy_bias,'b1'] += 1.
    data_block[section_names.galaxy_rsd,'fsig'] = 0.5
    pybird.execute()
    pk2 = data_block[section_names.model,'y_callable'](k,mu)
