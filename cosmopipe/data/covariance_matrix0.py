...
def setup(name, config_block, data_block):
    ...
    if options.get_string('covariance_file',None) is not None:
        ...
        cov = data_block[section_names.covariance,'covariance_matrix']
    from .data_vector import get_kwview
    projs = get_kwview(cov.x[0],options.get('data',{}))
