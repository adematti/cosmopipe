...
from cosmopipe.lib.parameters import ParamName
...
class GaussianLikelihoodFromSamples(GaussianLikelihood):
    ...
    def set_samples(self):
        if self.samples_file:
            samples = Samples.load_auto(self.samples_file)
        else:
            samples = self.pipe_block[self.samples_key]
        if self.parameters is None:
            self.parameters = samples.parameters.select(fixed=False)
            self.parameter_names = [str(param.name) for param in self.parameters]
        else:
            self.parameter_names = [str(ParamName(param)) for param in self.parameters]
        ...
        self.pipe_block[section_names.data,'y'] = samples.mean(columns=self.parameters)
