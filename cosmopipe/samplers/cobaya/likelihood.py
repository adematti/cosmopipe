...
class CosmopipeLikelihood(CobayaLikelihood):
    ...
    renames = {}
    # For classy...
    renames['cosmological_parameters.Omega_c'] = 'Omega_cdm'
    renames['cosmological_parameters.omega_c'] = 'omega_cdm'
    ...
    @property
    def set_linear_perturbations(self):
        return 'linear_perturbations' in self.requirements
    ...
    def get_requirements(self):
        ...
        self.z = 1./a - 1
        if self.set_linear_perturbations:
    ...
    def logp(self, **kwargs):
        ...
        if self.set_linear_perturbations:
            ...
            self._pipeline.pipe_block[section_names.linear_perturbations,'pk_callable'] = PkLinear(k=k/h,pk=pk*h**3)
            self._pipeline.pipe_block[section_names.background,'growth_rate'] = f = self.theory.get_fsigma8(self.z)/self.theory.get_sigma8(self.z)
        ...
        if self.set_background:
            ...
            self._pipeline.pipe_block[section_names.linear_perturbations,'sound_horizon_drag'] = float(self.provider.get_param('rdrag'))*h
            self._pipeline.pipe_block[section_names.background,'hubble_rate'] = hubble/h
